			...
			{!transaction ? (
				<>
					...
					<div className='flex flex-col text-right dark:text-blue-200'>
						...
						<p className='text-gray-400 font-semibold dark:text-gray-200'>
							US$ {price * hardcodedUserBalance[symbol]}
						</p>
						{priceChange > 0 ? (
							<p className='text-green-500'>{priceChange}%</p>
						) : (
							<p className='text-red-500 dark:text-red-300'>{priceChange}%</p>
						)}
					</div>
				</>
			) : (
				<div className='inline-block w-128 sm:w-80 dark:text-blue-200'>
					<div className='flex justify-between sm:flex-col'>
						...
						<div
							className={`flex flex-col sm:flex-row text-right sm:w-max sm:mt-2 ${
								buy ? 'text-green-500' : 'text-red-500 dark:text-red-300'
							}`}
						>
